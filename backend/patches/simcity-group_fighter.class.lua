	...
	if nMapIndex >= 0 then
		...
		if nNpcIndex > 0 then
			...
			if kind ~= 0 then 
				...
				if SetNpcLevel then SetNpcLevel(nNpcIndex, SIMBOT_LEVEL) end
				tbNpc.lastPos = {
					nX32 = tX*32,
					nY32 = tY*32,
					nPosId = nPosId
	...
	for i=1,N do
		...
		if nNpcIndex > 0 then
			...
			if kind ~= 0 then 
				...
				if SetNpcLevel then SetNpcLevel(nNpcIndex, SIMBOT_LEVEL) end   -- [2026-06-25] ep level 95 (GroupFighter chua bao gio SetNpcLevel -> children m_Level=4 -> cast/skill-render sai, va dong bo voi parent)
	...
	if tbNpc.playerID == 0 then
		...
		if tbNpc.walkMode == "random" or tbNpc.walkMode == "keoxe" or (not tbNpc.children) then
			if self:_arrived(nNpcIndex, tbNpc) then
				...
				if (tbNpc.noStop == 1 or random(1,100) < (SIMBOT_KEEP_WALKING_PCT or 90)) then
